...
class GridIndex {
     ...
     * 索引包含的所有点集合（哈希表）（引用）
     */
    public HashMap<String,SnapObject> pointsHash;
    /**
     * 索引包含的所有点集合（列表）（引用）
     */
    private List<SnapObject> pointsList;
    /**
     ...
     * @param ps 需要建立索引的点集
     */
    public GridIndex(MBR mapScale,double side, List<SnapObject> ps){
        this.side = side;
        //将点的引用加进来
        pointsHash = new HashMap<String,SnapObject>();
        for(int i=0;i<ps.size();i++){
            pointsHash.put(ps.get(i).id,ps.get(i));
        }
        pointsList = ps;
        //初始化经纬度边界
        initLngLat(mapScale);
     ...
     * @return 集合副本
     */
    public List<SnapObject> getPointsFromGrid(int indexLng,int indexLat){
        List<SnapObject> points = new ArrayList<SnapObject>();
        //溢出检查
...
//            return points;
     ...
     * @return 附近的点集合
     */
    public List<SnapObject> getPointsFromGrids(SnapObject p,int n){
        List<SnapObject> points = new ArrayList<SnapObject>();
        //溢出检查
        if(n<0)
            return points;
        ...
        for(int i=indexLng-n;i<=indexLng+n;i++){
            for(int j=indexLat-n;j<=indexLat+n;j++){
                List<SnapObject> ps = getPointsFromGrid(i,j);
    ...
    private void fillGrids(){
        for(int i=0;i<pointsList.size();i++){
            ...
            grids[indexPoint.indexLng][indexPoint.indexLat].substances.add(pointsList.get(i).id);
        }
    }
     ...
     * @param p 要删去的点
     */
    public void delete(SnapObject p){
        IndexPoint indexPoint = getIndex(p);
        //删去网格中的索引
        List<String> sub = grids[indexPoint.indexLng][indexPoint.indexLat].substances;
        for(int i=0;i<sub.size();i++){
            if(sub.get(i).equals(p.id))
                sub.remove(i);
        }
        //删去字典中的这个点
        pointsHash.remove(p.id);
