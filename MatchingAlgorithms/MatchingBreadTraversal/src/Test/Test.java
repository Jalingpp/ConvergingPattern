...
public class Test {
     public static void main(String[] args) throws IOException {
          ...
          String convergingoutpath = "/root/Converging_jjp/data/events_baseline/";
...
//          }
     ...
     public static HashMap<String, Event>  getAllEvents(ArrayList<TimeClusters> Tc,HashMap<String,HashMap<String,ArrayList<String>>> edges_clusters,RoadNetWork road,int minScale,int minDuration,int timeinterval){
          ...
          for(int i=0;i<Tc.size()-1;i++) {
               for(Map.Entry<String,Cluster> entry: Tc.get(i).getClusters().entrySet()) {
                    List<String> _edgelist = entry.getValue().getEdge();  //获取当前簇所在的边集
                    HashMap<String,Boolean> nextTExtendedEdge = new HashMap<>();    //记录下一时刻已经扩展过的边
                    String nextTimestamp = Tc.get(i+1).getTime();      //后一时刻时间戳字符串
                    String father = null;
                    List<String> _candidateClusters;
                    while(father==null){
                         List<String> _adjaEdges = new ArrayList<>();    //当前边集的邻边集合
                         _candidateClusters = getMatchCandidates(nextTimestamp,_edgelist,edges_clusters,road,_adjaEdges);  //获取当前簇的待匹配候选集
//                         System.out.println("_adjaEdges.size() = "+_adjaEdges.size());
                         for(int j=0;j<_adjaEdges.size();j++){
                              if(!nextTExtendedEdge.containsKey(_adjaEdges.get(j))){   //若未验证，则加入到验证列表中
                                   nextTExtendedEdge.put(_adjaEdges.get(j),true);
                              }else{  //若已验证，在候选列表中删除当前簇ID
                                   _adjaEdges.remove(j);
                                   j--;
                              }
                         } //判断候选簇是否被验证过
//                         System.out.println("_adjaEdges.size() = "+_adjaEdges.size());
                         if(_adjaEdges.size()==0){
                              break;
                         }  //如果候选列表为空，退出循环
                         father = getFatherCluster(entry.getValue(),_candidateClusters,Tc.get(i+1));  //获取当前簇的父簇
                         //建立包含关系
                         if(father!=null){
                              String _rootkey = (i+1)+"-"+father;
                              rootClustersList.add(_rootkey);
                              rootClustersMap.put(_rootkey,1);
                              //设置当前簇的父簇，并将其加入到父簇的子簇表中
                              entry.getValue().setFatherId(_rootkey);
                              String _childkey = i+"-"+entry.getValue().getCid();
                              Tc.get(i+1).getClusters().get(father).getChildClusters().add(_childkey);
                              //修改父簇的叶节点时间（事件最早起始时间）
                              Tc.get(i+1).getClusters().get(father).setEventStartTime(entry.getValue().getEventStartTime());
                              //判断当前簇曾经是不是，若是，则在根节点表中删除
                              String _queryRootkey = i+"-"+entry.getValue().getCid();
                              if(rootClustersMap.get(_queryRootkey)!=null)
                                   rootClustersMap.put(_queryRootkey,0);
                         }
                         _edgelist = _adjaEdges;
                    }
